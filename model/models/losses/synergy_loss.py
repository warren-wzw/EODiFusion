...
class SynergyLoss(nn.Module):
    ...
    def forward(self, fused_img, ir_img, rgb_img, seg_pred):
        loss = dict() 
        # 软分割掩码
        seg_mask = F.softmax(seg_pred, dim=1)
        fg_mask = torch.sum(seg_mask[:, 1:, :, :], dim=1, keepdim=True)
        bg_mask = seg_mask[:, 0:1, :, :]
        
        # 协同损失：分割指导融合选择
        synergy_fg = F.mse_loss(fused_img * fg_mask, ir_img * fg_mask)
        synergy_bg = F.mse_loss(fused_img * bg_mask, rgb_img * bg_mask)
        
        # 权重可调的协同损失
        synergy_loss = self.alpha*synergy_fg + self.beta*synergy_bg
        
        loss["synergy_loss"] = synergy_loss
