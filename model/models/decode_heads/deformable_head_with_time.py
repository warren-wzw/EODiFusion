...
class DeformableHeadWithTime(BaseDecodeHead):
    ...
    def forward_fusion(self, inputs, times,img_ori,ir_ori):
        memory = self.forward(inputs, times)#[b,numclssses,80,120]
        input_high  = torch.cat([img_ori, ir_ori], dim=1) #[b,4,h,w]
        fusion_out = self.reconstruct(memory,input_high)#[batch,256,h/4,w/4]->[batch,1,h,w]
        return fusion_out,memory
